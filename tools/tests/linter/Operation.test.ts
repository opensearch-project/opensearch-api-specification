...
test('validate_response()', () => {
  ...
  const invalid_responses = operation({
    'x-operation-group': 'cat.info',
    responses: {
...
test('validate_path_parameters()', () => {
  ...
  const valid_path_params = operation({
    parameters: [
      { $ref: '#/components/parameters/indices.create::path.index' },
...
test('validate()', () => {
  ...
  const typical_invalid = mocked_operation({
    validate_group: undefined,
    validate_namespace: undefined,
    validate_operationId: 'Invalid operationId',
    validate_description: 'Invalid description',
    validate_requestBody: 'Invalid requestBody',
    validate_responses: ['Invalid response 1', 'Invalid response 2'],
    validate_parameters: 'Invalid parameters',
    validate_path_parameters: 'Invalid path parameters'
  })
