...
export default class OperationGroup extends ValidatorBase {
  ...
  validate_description(): ValidationError | void {
    ...
    if(uniq_descriptions.size > 1)
      return this.error(`${this.operations.length} '${this.name}' operations must have identical description property.`);
  }
  ...
  validate_externalDocs(): ValidationError | void {
    ...
    if(uniq_externalDocs.size > 1)
      return this.error(`${this.operations.length} '${this.name}' operations must have identical externalDocs property.`);
  }

  validate_requestBody(): ValidationError | void {
    const uniq_requestBodies = new Set(this.operations.map((op) => op.spec.requestBody?.$ref));
    if(uniq_requestBodies.size > 1)
      return this.error(`${this.operations.length} '${this.name}' operations must have identical requestBody property.`);
  }
  ...
  validate_responses(): ValidationError | void {
    ...
    if(uniq_signatures.size > 1)
      return this.error(`${this.operations.length} '${this.name}' operations must have an identical set of responses.`);
  }
  ...
  validate_query_parameters(): ValidationError | void {
    ...
    if(uniq_signatures.size > 1)
      return this.error(`${this.operations.length} '${this.name}' operations must have an identical set of query parameters.`);
  }
}