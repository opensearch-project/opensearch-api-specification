...
export default class RootFile extends FileValidator {
  ...
  validate_paths (): ValidationError[] {
    return Object.entries(this.spec().paths).map(([path, spec]) => {
      if (!spec?.$ref) { return this.error('Every path must be a reference object to a path in a namespace file.', `Path: ${path}`) }
  ...
  validate_params (): ValidationError[] {
    ...
    return Object.entries(params).map(([name, param]) => {
      ...
      if (!param['x-global']) { return this.error('Parameters in root file must have \'x-global\' extension set to true.', `#/components/parameters/${name}`) }
    }).filter((e) => e) as ValidationError[]
  }
}
